...
class _PlacementStrategyLocation(_PlacementStrategy):
    ...
    def _try_place_object_internal(self, obj, target_obj, resolver, ignored_object_ids=None, **kwargs):
        ...
        if self.allow_off_lot_placement and not active_lot.is_position_on_lot(start_position):
            ...
            context = create_fgl_context_for_object_off_lot(starting_location, obj, terrain_tags=terrain_tags, search_flags=search_flags, ignored_object_ids=(obj.id,), restrictions=restrictions, min_water_depth=self.min_water_depth, max_water_depth=self.max_water_depth, routing_context=routing_context, scoring_functions=scoring_functions, **raytest_kwargs)
        else:
            ...
            context = create_fgl_context_for_object(starting_location, obj, terrain_tags=terrain_tags, search_flags=search_flags, ignored_object_ids=ignored_object_ids, position_increment_info=pos_increment_info, restrictions=restrictions, min_water_depth=self.min_water_depth, max_water_depth=self.max_water_depth, routing_context=routing_context, scoring_functions=scoring_functions, **raytest_kwargs)
