...
class ObjectDefinitionPickerInteraction(ObjectPickerInteraction):
    ...
    def create_row(cls, inst, row_obj, context=DEFAULT, target=DEFAULT):
        ...
        row = ObjectPickerRow(object_id=row_obj.id, def_id=row_obj.id, icon_info=icon_info, tag=row_obj, tag_list=tag_list, name=LocalizationHelperTuning.get_object_name(row_obj))
