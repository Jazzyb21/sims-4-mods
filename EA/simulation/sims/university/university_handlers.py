...
def get_not_yet_accepted_degrees(sim_info, university_manager, degree_manager):
    ...
    for (university_id, degree_ids) in not_yet_accepted_degrees.items():
        ...
        for degree_id in degree_ids:
            ...
            degrees_data.append({'degree': degree.__name__, 'university': university.__name__, 'prestige': is_prestige, 'degree_score': str(degree.acceptance_score.score_threshold.value) if is_prestige else 'Not Required', 'sim_score': str(degree.get_sim_acceptance_score(sim_info, degree.acceptance_score)) if is_prestige else '', 'is_qualify': degree.can_sim_be_accepted(sim_info, degree.acceptance_score) if is_prestige else True})
    return degrees_data
