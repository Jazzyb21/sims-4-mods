...
class DecoratorGig(ActiveGig):
    ...
    def apply_new_object_tags(self):
        if services.current_zone_id() != self._customer_lot_id or not self.decorator_gig_tuning.new_object_tags:
            return
    ...
    def create_picker_row(cls, owner=None, gig_customer=None, gig_budget=None, enabled=True, **kwargs):
        ...
        row = ObjectPickerRow(name=cls.display_name(owner), icon=cls.display_icon, row_description=description, row_tooltip=row_tooltip, rarity_text=rarity_text, second_rarity_text=second_rarity_text, flair_icon=cls._decorator_gig_tuning.decorator_gig_type_icon.icon, flair_name=cls._decorator_gig_tuning.decorator_gig_type_icon.name(), is_enable=enabled)
    ...
    def clean_up_gig(self):
        ...
        if self._owner and self._owner.career_tracker:
    ...
    def uses_gig_notifications_for_promotions(self):
        return False
lock_instance_tunables(DecoratorGig, additional_pay_per_overmax_level=None, gig_cast_rel_bit_collection_id=None, gig_cast=[], end_of_gig_overmax_notification=None, end_of_gig_overmax_rewardless_notification=None, odd_job_tuning=None)