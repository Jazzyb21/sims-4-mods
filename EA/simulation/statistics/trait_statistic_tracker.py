...
class TraitStatisticTracker(ContinuousStatisticTracker):
    ...
    def add_statistic(self, stat_type, from_load=False, **kwargs):
        ...
        if not from_load:
            ...
            if stat.trait_unlocked or stat.group != TraitStatisticGroup.NO_GROUP and (self._trait_statistic_groups is not None and stat.group in self._trait_statistic_groups) and len(self._trait_statistic_groups[stat.group]) >= TraitStatistic.GROUPS[stat.group]:
    ...
    def _on_statistic_state_changed(self, changed_statistic):
        ...
        if changed_statistic.trait_unlocked:
            ...
            if self._trait_statistic_groups[group_being_changed] or currently_locked:
                if self._trait_statistic_groups is not None and len(self._trait_statistic_groups[group_being_changed]) >= TraitStatistic.GROUPS[group_being_changed]:
                    return
                for statistic in self._statistics.values():
                    if statistic.group != group_being_changed:
                        pass
