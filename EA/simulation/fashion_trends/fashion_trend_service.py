...
class FashionTrendService(Service, ComponentContainer, HasStatisticComponent):
    ...
    def get_outfit_trend(self, outfit_data):
        if hasattr(outfit_data, 'trend') and outfit_data.trend != 0:
    ...
    def get_outfit_prevalent_trend_tag(self, target):
        ...
        if target.is_sim:
            ...
            if sim_outfits.has_outfit_category(OutfitCategory.SPECIAL):
    ...
    def apply_sold_fashion_outfit(self, target_sim):
        if target_sim is not None and target_sim.is_sim:
            ...
            if sold_outfit_mannequin is not None:
                ...
                if outfits is not None:
                    ...
                    target_sim_info.generate_merged_outfit(sim_info_source, (OutfitCategory.SITUATION, 0), (current_outfit_category, current_outfit_index), (OutfitCategory.EVERYDAY, 0))
    ...
    def get_mannequin(self, gender, sim_id=0):
        if self.thrift_store_mannequin is not None and self.thrift_store_mannequin.gender == gender:
    ...
    def _schedule_fashion_trend_shift(self, fashion_trend=None):
        ...
        self._processor_trend_shift_schedule[fashion_trend] = sim_timeline.schedule(elements.GeneratorElement(self._process_trend_shift_schedule_gen(fashion_trend)), when=schedule_time)

    def _setup_daily_thrift_store_refresh_alarm(self):
        day_time = date_and_time.create_date_and_time(hours=int(FashionThriftStoreTuning.DAILY_REFRESH_TIME.hour()), minutes=int(FashionThriftStoreTuning.DAILY_REFRESH_TIME.minute()))
        ...
        sim_timeline = services.time_service().sim_timeline
        self._processor_daily_schedule = sim_timeline.schedule(elements.GeneratorElement(self._process_daily_schedule_gen), when=schedule_time)

    def _setup_exclusive_item_unlock_alarm(self):
        day_time = date_and_time.create_date_and_time(hours=int(FashionThriftStoreTuning.EXCLUSIVE_ITEM_NOTIFICATION_TIME.hour()), minutes=int(FashionThriftStoreTuning.EXCLUSIVE_ITEM_NOTIFICATION_TIME.minute()))
        now = services.time_service().sim_now
        time_delay = now.time_till_next_day_time(day_time, True)
        if time_delay.in_ticks() == 0:
            time_delay = date_and_time.create_time_span(days=1)
        schedule_time = now + time_delay
        sim_timeline = services.time_service().sim_timeline
        self._processor_daily_unlock_schedule = sim_timeline.schedule(elements.GeneratorElement(self._process_daily_unlock_schedule_gen), when=schedule_time)
    ...
    def _randomize_thrift_store_inventory(self):
        ...
        for cas_part_type in FashionThriftStoreTuning.REFRESH_CAS_PART_LIST:
            if self._exclusive_item_unlocked or cas_part_type.is_exclusive:
                pass
            else:
                ...
                cas_parts_list = randomize_caspart_list(include_tags=list(include_tags), exclude_tags=list(exclude_tags), bodytype=body_type, count=count, seed=seed)
    ...
    def _process_trend_shift_schedule_gen(self, fashion_trend=None):
        ...
        def _process_specific_trend_schedule_gen(timeline):
            try:
                ...
                logger.exception('Exception while scheduling trend shifts: ', exc=exception, level=sims4.log.LEVEL_ERROR)
            finally:
                if not teardown:
    ...
    def _process_daily_schedule_gen(self, timeline):
        try:
            ...
            logger.exception('Exception while scheduling daily thrift store refresh: ', exc=exception, level=sims4.log.LEVEL_ERROR)
        finally:
            if not teardown:
                self._setup_daily_thrift_store_refresh_alarm()
    ...
    def _process_daily_unlock_schedule_gen(self, timeline):
        try:
            teardown = False
            if FashionThriftStoreTuning.EXCLUSIVE_ITEM_NOTIFICATION and self._exclusive_item_unlocked:
                active_sim_info = services.active_sim_info()
            ...
            logger.exception('Exception while scheduling daily exclusive unlock notification: ', exc=exception, level=sims4.log.LEVEL_ERROR)
        finally:
            if not teardown:
                self._setup_exclusive_item_unlock_alarm()
