...
class VenueService(Service):
    ...
    def get_zones_for_venue_type_gen(self, *venue_types, compatible_region=None, ignore_region_compatability_tags=False, region_blacklist=[]):
        for lot_owner_info in services.get_persistence_service().get_lots_proto_buff_gen():
            ...
            if self.get_zone_venue_type_valid_for_venue_types(zone_id, venue_types, compatible_region=compatible_region, ignore_region_compatability_tags=ignore_region_compatability_tags, region_blacklist=region_blacklist) is not None:
                yield zone_id
    ...
    def get_zone_and_venue_type_for_venue_types(self, venue_types, compatible_region=None):
        ...
        for lot_owner_info in services.get_persistence_service().get_lots_proto_buff_gen():
            zone_id = lot_owner_info.zone_instance_id
            venue_type = self.get_zone_venue_type_valid_for_venue_types(zone_id, venue_types, compatible_region=compatible_region)
            if venue_type is not None:
...
class VenueGameService(Service):
    ...
    def on_cleanup_zone_objects(self, client):
        ...
        for provider in self._zone_provider.values():
            provider.finalize_startup()
        if self._venue_restore_zone_id is not None and self._venue_restore_zone_id != services.current_zone_id():
        ...
        for zone_data_msg in zones:
            if zone_data_msg is None:
                ...
                if active_venue_tuning_id is None:
                    ...
                    if source_venue_type is None:
                        self.set_provider(zone_data_msg.zone_id, None)
                    elif source_venue_type.variable_venues is None or not (source_venue_type.variable_venues is not None and source_venue_type.variable_venues.enable_civic_policy_support):
                        self.set_provider(zone_data_msg.zone_id, None)
                    else:
    ...
    def load(self, zone_data=None):
        ...
        if save_slot_data_msg.gameplay_data.HasField('venue_game_service'):
            ...
            if self._venue_restore_type_id == services.venue_service().get_venue_tuning(self._venue_restore_zone_id).guid64:
                self._venue_restore_alarm_handler = alarms.add_alarm(self, TimeSpan(venue_game_data.venue_restore_alarm_time), self._restore_venue_type_delay_handler, repeating=False)
            else:
                self._venue_restore_zone_id = None
                self._venue_restore_type_id = None
    ...
    def restore_venue_type(self, zone_id, delay):
        if delay and zone_id == services.current_zone_id():
            self._venue_restore_alarm_handler = alarms.add_alarm(self, delay, self._restore_venue_type_delay_handler, repeating=False)
