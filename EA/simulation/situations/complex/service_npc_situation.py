...
class ServiceNpcSituation(SituationComplexCommon):
    ...
    def _on_remove_sim_from_situation(self, sim):
        if not (sim is self.service_sim() and (self._is_recurring or self._is_leaving or services.current_zone().service_npc_service is not None and self._service_npc_type.max_work_duration() == 0 and services.current_zone().is_zone_shutting_down)):
