...
class GatherState(CommonInteractionCompletedSituationState):
    ...
    def _on_set_sim_role_state(self, sim, job_type, role_state_type, role_affordance_target):
        ...
        if job_type is not self.owner.head_crafter:
            return
        context = InteractionContext(sim, InteractionContext.SOURCE_SCRIPT, Priority.Critical, run_priority=Priority.High, insert_strategy=QueueInsertStrategy.NEXT)
...
class CraftState(CommonSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        result = crafting_sim.push_super_affordance(self._crafting_interaction, self.owner._target, context, start_crafting_interaction=self.owner._start_crafting_interaction, recipe=self.owner._recipe, crafter=crafting_sim, ordering_sim=self.owner._ordering_sim, crafting_target=self.owner._crafting_target, orderer_ids=self.owner._orderer_ids, ingredients=self.owner._ingredients, recipe_funds_source=self.owner._funds_source, paying_sim=self.owner._paying_sim, ingredient_cost_only=self.owner._ingredient_cost_only, situation_id=self.owner.id)
...
class GroupCraftingSituation(SituationComplexCommon):
    ...
    def _states(cls):
        if cls.stop_other_crafters_state is None:
            return (SituationStateData.from_auto_factory(1, cls.gather_state), SituationStateData.from_auto_factory(2, cls.craft_state))
        return (SituationStateData.from_auto_factory(1, cls.gather_state), SituationStateData.from_auto_factory(2, cls.craft_state), SituationStateData.from_auto_factory(3, cls.stop_other_crafters_state))
    ...
    def _on_remove_sim_from_situation(self, sim):
        ...
        for si in itertools.chain(sim.si_state, sim.queue):
            ...
            elif type(si) in self.interactions_to_cancel_on_removal:
                si.cancel(FinishingType.SITUATIONS, 'SI removal on Sim leaving group crafting situation.')
