...
class _AwardsStatesBase(CommonInteractionCompletedSituationState):
    ...
    def _on_interaction_of_interest_complete(self, resolver=None, **kwargs):
        if services.current_zone().is_in_build_buy:
            return
        if resolver is not None and (resolver.interaction.has_been_reset or resolver.interaction.has_been_canceled):
...
class _PresenterGoToPodiumState(_AwardsStatesBase):
    ...
    def on_activate(self, reader=None):
        ...
        if self.owner.is_running and self.owner.get_target_object() is None:
            self._change_state(self.owner.announce_awards_in_route_fail_state())
...
class PromSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'gather_and_dance_state': _GatherAndDanceState.TunableFactory(description='\n            A state where sims in the situation gathers and dance near stereos.\n            ', tuning_group=GroupNames.STATE, display_name='01_gather_and_dance_state'), 'presenter_goto_podium_state': _PresenterGoToPodiumState.TunableFactory(description='\n            A situation state where the presenter routes to the podium in preparation\n            for the awards ceremony. If the presenter fails to route to the podium, the\n            state will go to announce_awards_in_route_fail_state.\n            ', tuning_group=GroupNames.STATE, display_name='02_presenter_go_to_podium_state'), 'prepare_for_awards_state': _PrepareForAwardsState.TunableFactory(description='\n            A situation state that gathers the sim by the podium to prepare for awards.\n            If the podium cannot be found, the state will go to announce_awards_in_route_fail_state.\n            ', tuning_group=GroupNames.STATE, display_name='03_teens_prepare_for_awards_state'), 'present_awards_state': _PresentAwardsState.TunableFactory(description='\n            A state where the prom royalty and jester winners are announced via a TNS.\n            The state will go to announce_awards_in_route_fail_state if presenting interaction gets cancelled.\n            ', tuning_group=GroupNames.STATE, display_name='04_present_awards_state'), 'accept_awards_state': _AcceptAwardsState.TunableFactory(description='\n            A state where the winners goes near the podium and spins into crown/jester hat.\n            If the winners fails to route to the podium, the situation will go to accept awards in route fail state.\n            ', tuning_group=GroupNames.STATE, display_name='05_accept_awards_state'), 'announce_awards_in_route_fail_state': _AnnounceAwardsInRouteFailState.TunableFactory(description='\n            A situation state where the presenter, teens fails to route to the podium,\n            or the podium is not found. The winners notification will be pushed and teens will not gather.\n            ', tuning_group=GroupNames.STATE, display_name='06_announce_awards_in_route_fail_state'), 'accept_awards_in_route_fail_state': _AcceptAwardsInRouteFailState.TunableFactory(description='\n            A state after the route fail state where the awards winners will be forced to \n            spin into crown where they are standing, usually in the case where we cannot find the podium.\n            ', tuning_group=GroupNames.STATE, display_name='07_accept_awards_in_route_fail_state'), 'more_dancing_state': _MoreDancingState.TunableFactory(description='\n            A state where teens go back to dancing and socializing.\n            ', tuning_group=GroupNames.STATE, display_name='08_more_dancing_state'), 'chaperone_situation_job': TunableSituationJobAndRoleState(description='\n            Job and role for chaperones at prom.\n            ', tuning_group=GroupNames.ROLES), 'announcer_situation_job': TunableSituationJobAndRoleState(description='\n            Job and role for the announcer.\n            ', tuning_group=GroupNames.ROLES), 'teens_situation_job': TunableSituationJobAndRoleState(description='\n            Job and role for teens that attend prom.\n            ', tuning_group=GroupNames.ROLES), 'active_sims_job': TunableSituationJobAndRoleState(description='\n            Job and role for the instanced active household sims.\n            ', tuning_group=GroupNames.ROLES), 'per_vote_weight': TunableRange(description='\n            The weight of a single vote the sim receives. This weight will affect the chance\n            of a sim winning a prom royalty/jester title.\n            ', tunable_type=float, default=1.2, minimum=1.0, tuning_group=GroupNames.SITUATION), 'prom_royalty_vote_statistic': TunableReference(description='\n            The statistic that we will use in order to determine how\n            many nominations the sim received for prom royalty title.\n            ', manager=services.get_instance_manager(sims4.resources.Types.STATISTIC), tuning_group=GroupNames.SITUATION), 'prom_jester_vote_statistic': TunableReference(description='\n            The statistic that we will use in order to determine how\n            many nominations the sim received for prom jester title.\n            ', manager=services.get_instance_manager(sims4.resources.Types.STATISTIC), tuning_group=GroupNames.SITUATION), 'prom_royalty_multiplier': TunableMultiplier.TunableFactory(description='\n            A multiplier to the weight for the sims in the situation to win the prom\n            royalty title if they passed these tests.\n            ', tuning_group=GroupNames.SITUATION), 'prom_jester_multiplier': TunableMultiplier.TunableFactory(description='\n            A multiplier to the weight for the sims in the situation to win the prom\n            jester title if they passed these tests.\n            ', tuning_group=GroupNames.SITUATION), 'podium_tag': TunableTags(description='\n            Tags used to find the podium which the sims will gather around in the \n            present awards state.\n            ', tuning_group=GroupNames.SITUATION), 'royalty_winner_role_state': RoleState.TunableReference(description='\n            A role state the prom royalty winner will use to spin into crown. This will be pushed through code.\n            ', tuning_group=GroupNames.SITUATION), 'jester_winner_role_state': RoleState.TunableReference(description='\n            A role state the prom jester title winner will use to spin into crown. This will be pushed through code.\n            ', tuning_group=GroupNames.SITUATION), 'prom_winners_notification': UiDialogNotification.TunableFactory(description='\n            A TNS that is displayed to announce the prom royalty and jester winners.\n            ', tuning_group=GroupNames.SITUATION), 'post_prom_drama_node': TunableReference(description='\n            A drama node used to trigger the post prom party invite.\n            ', manager=services.get_instance_manager(sims4.resources.Types.DRAMA_NODE), tuning_group=GroupNames.SITUATION), 'post_prom_schedule_delay': TunableSimMinute(description='\n            Post prom drama node will trigger after this amount of time in sim minutes after\n            prom ends. This number should be less than the venue switch timer so the screens\n            wont happen at the same time.\n            ', default=10, tuning_group=GroupNames.SITUATION)}
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, _GatherAndDanceState, factory=cls.gather_and_dance_state), SituationStateData(2, _PresenterGoToPodiumState, factory=cls.presenter_goto_podium_state), SituationStateData(3, _PrepareForAwardsState, factory=cls.prepare_for_awards_state), SituationStateData(4, _PresentAwardsState, factory=cls.present_awards_state), SituationStateData(5, _AcceptAwardsState, factory=cls.accept_awards_state), SituationStateData(6, _AnnounceAwardsInRouteFailState, factory=cls.announce_awards_in_route_fail_state), SituationStateData(7, _AcceptAwardsInRouteFailState, factory=cls.accept_awards_in_route_fail_state), SituationStateData(8, _MoreDancingState, factory=cls.more_dancing_state))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.chaperone_situation_job.job, cls.chaperone_situation_job.role_state), (cls.announcer_situation_job.job, cls.announcer_situation_job.role_state), (cls.teens_situation_job.job, cls.teens_situation_job.role_state), (cls.active_sims_job.job, cls.active_sims_job.role_state)]
    ...
    def _situation_timed_out(self, *args, **kwargs):
        ...
        if drama_scheduler is not None:
            ...
            (drama_scheduler.schedule_node(self.post_prom_drama_node, resolver, specific_time=time),)
    ...
    def generate_prom_winners(self):
        if self.prom_jester_winner_id and self.prom_royalty_winner_id:
            ...
            if royalty_info and (self.is_sim_info_in_situation(royalty_info) and jester_info) and self.is_sim_info_in_situation(jester_info):
                return
        ...
        for sim in itertools.chain(self.all_sims_in_job_gen(self.teens_situation_job.job), self.all_sims_in_job_gen(self.active_sims_job.job)):
            ...
            if royalty_votes_stat is not None and royalty_votes_stat.get_value() != 0:
                weighted_sims_royalty.append((royalty_votes_stat.get_value()*self.per_vote_weight*royalty_multiplier, sim_id))
            else:
                weighted_sims_royalty.append((royalty_multiplier, sim_id))
            if jester_votes_stat is not None and jester_votes_stat.get_value() != 0:
                weighted_sims_jester.append((jester_votes_stat.get_value()*self.per_vote_weight*jester_multiplier, sim_id))
            else:
    ...
    def assign_sim_winner_role(self, winner_id, winner_role_state):
        ...
        if winner is not None and self.is_sim_info_in_situation(winner):
    ...
    def push_winner_notification(self):
        ...
        dialog = self.prom_winners_notification(services.active_sim_info(), resolver=DoubleSimResolver(royalty_winner, jester_winner))
        dialog.show_dialog()
    ...
    def _on_podium_object_displaced(self, script_object, *args, **kwargs):
        if script_object is not self.podium:
            return
        if self.podium and self.podium.is_on_location_changed_callback_registered(self._on_podium_object_displaced):
