...
class BouncerSimData:
    ...
    def can_assign_to_request(self, new_request, check_exclusivity=True):
        ...
        for cur_request in self._requests:
            if new_request._reassign_within_situation or cur_request._situation is new_request._situation:
                return False
    ...
    def _get_excluded_requests(self, new_request, trump_all_exclusions=False):
        excluded = []
        for cur_request in self._requests:
            if new_request._reassign_within_situation and cur_request._situation is new_request._situation:
...
class Bouncer(sims.sim_spawner_service.ISimSpawnerServiceCustomer):
    ...
    def _apply_common_blacklists(self, request, blacklist, spawning_sim_ids, active_household_sim_ids, active_lot_household_sim_ids):
        ...
        elif request._for_persisted_sim:
        ...
        if filter_results:
            ...
            if not bouncer_request._for_persisted_sim:
                ...
                if during_zone_spin_up and bouncer_request.specific_location is not None:
                    ...
                    spawn_strategy = sims.sim_spawner_service.SimSpawnPointStrategy(spawner_tags=bouncer_request.spawner_tags(during_zone_spin_up), spawn_point_option=bouncer_request.spawn_point_option, spawn_action=bouncer_request._spawn_action, saved_spawner_tags=bouncer_request.saved_spawner_tags, spawn_at_lot=bouncer_request.spawn_at_lot, use_random_sim_spawner_tag=bouncer_request.use_random_sim_spawner_tag)
